...
- Event photo backgrounds — `.hero-bg` (index.html hero, the photo of whatever `pintarFondoHero()` picks as "evento destacado": the first upcoming event with a photo) and `.detail-bg` (the event detail page) — are the *one* place color is allowed to bleed through: the photo at `blur(72px)` (spec range 60–80px) with a flat `rgba(8,8,10,.75)` layer on top, **no `brightness()` filter and no gradient** — either would flatten the tint the photo is there to give (a sunset's orange, a map's gold). `inset:-80px` on both is overscan so the blur doesn't show a falloff edge at the container's bound — keep it ≥ the blur radius if you change either. No event photo → both fall back to the same treatment: the Bronx logo as `background-image`, `opacity:.05`, no blur/overlay (`.hero-bg.sin-foto` / `.detail-bg:not([style*="background-image"])`).
