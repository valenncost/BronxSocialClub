...
- The background is flat black plus a **luz de ambiente**: two static radial halos on `body::before` (fixed, `z-index:-1`, `pointer-events:none`) at alpha `0.02` orange and `0.012` pink (halved once already from an initial `0.055`/`0.035` — that's the direction "too strong" gets fixed in, don't raise them back). The intent is club lighting, not a glow. If a halo reads as a visible smudge it's too strong. **No blur and no animation on that layer**, and the old full-strength glows and the grain texture stay gone. **The hero's own `.hero-tinte` halos are the one deliberate exception** — per-slide request, they run hotter than the ambient halo precisely to read as club lighting behind the hero content; that boost is scoped to `.hero-tinte`, `body::before` itself is untouched. "Naranja" (`0.10`/`0.16`, ellipse anchored at `20% 0%`) sits on top of the hero's own orange logo (`.hero::before`, always visible at full opacity for this slide) and is meant as asymmetric ambient lighting, not full coverage — the logo does the rest of the work. "Rojo" went through two rounds of the same underlying bug (tinting a background that was still visibly gray) before landing on the fix: desaturating the logo and tinting it red on top still reads as "gray with a red tinge," never a clean red, no matter the alpha or the gradient's position/size — so `.hero[data-tinte="rojo"]::before{opacity:0}` now hides the logo outright for this slide (transitioned via the `opacity` added to `.hero::before`'s transition list) and the halo is the *entire* background, painted over the plain black `.hero::after` overlay. "Neutro" is the one slide that still desaturates-not-hides the logo (`.hero[data-tinte="neutro"]::before{filter:blur(72px) grayscale(1)}`), since it's supposed to read as a neutral gray, not black. **If a future color also needs to fully replace the logo's hue** (not just tint it), hide the logo (`opacity:0`) rather than desaturate-then-tint it — and sanity-check on a narrow mobile viewport with the title actually wrapped to two lines, not just desktop width, since that's where the halo's shape/size matters most.
