...
The palette is **black + white + glow**: a black base with translucent veils on top, neutral white/gray type and controls, and glow instead of flat color. This applies to the **public site only** (index/detail/checkout, cuenta.html, entradas.html) — brand orange isn't the default there any more, the only color comes from the per-event accent (see "Per-event accent color").

**The Studio (`admin.html`) and the door scanner (`escaner.html`) are scoped back to brand orange**, on purpose: `body[data-page="admin"], body[data-page="escaner"] { --accent:#F58C29; --acento-rgb:245,140,41; --acento-par:#F7931E }`, right after "COLOR DE ACENTO POR EVENTO" in `css/estilos.css` — same mechanism as `[data-color-evento]`, just scoped to each page's `<body>` instead of an event container, and it redefines nothing else: every rule that already reads `var(--accent)`, `rgba(var(--acento-rgb),a)`, or `var(--glow-suave)`/`var(--glow-fuerte)` (which themselves read `--acento-rgb`) re-tints for free — no duplicated button/border/focus rules for "Studio orange" vs. "public neutral". `escaner.html` is included because it renders the shared public `<nav>` (index.html doesn't reuse it there — `admin.html` has its own dashboard chrome and never loads that `<nav>` at all) and the scanner counts as Studio chrome, not public site. `[data-color-evento]` still wins wherever it's actually present (never inside either of these two pages) since it sets the variables directly on a more specific element, not through inheritance from `<body>`.
