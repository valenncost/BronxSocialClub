...
- The background is flat black plus a **luz de ambiente**: two static radial halos on `body::before` (fixed, `z-index:-1`, `pointer-events:none`) at alpha `0.02` orange and `0.012` pink (halved once already from an initial `0.055`/`0.035` — that's the direction "too strong" gets fixed in, don't raise them back). The intent is club lighting, not a glow. If a halo reads as a visible smudge it's too strong. **No blur and no animation on that layer**, and the old full-strength glows and the grain texture stay gone. **The hero's own `.hero-tinte` halos are the one deliberate exception** — per-slide request, they run hotter than the ambient halo precisely to read as club lighting behind the hero content; that boost is scoped to `.hero-tinte`, `body::before` itself is untouched. The two slides aren't at the same alpha or the same shape, and that's deliberate: "naranja" (`0.10`/`0.16`, ellipse anchored at `20% 0%`) sits on top of the hero's own orange logo and is meant as asymmetric ambient lighting, not full coverage — the logo does the rest of the work. "Rojo" desaturates that same logo to gray first (see the carousel bullet below) and has to carry 100% of the red itself, so besides running hotter (`0.30`/`0.42`) its main halo is also centered and much larger (`ellipse 95% 75% at 50% 15%`, not `20% 0%`) — copying "naranja"'s corner-anchored shape at a higher alpha still faded out before it reached the headline on a tall/wrapped mobile hero, leaving the title itself sitting on the untinted zone and reading as identical to "neutro" (the actual bug, twice: first the alpha, then the shape). If you add a fourth color that has to carry its own tint the way "rojo" does (anything that also desaturates the logo), give it "rojo"'s centered/larger shape, not "naranja"'s corner one — and check the halo actually reaches behind the `<h1>` on a narrow mobile viewport before calling it done, not just at desktop width.
...
- Accent light is allowed only in these places, at these strengths: card hover border `0.25` (`.tipo-card`, `.evento-card`, `.ev-admin-item` — the Studio/detail surfaces that still have a background and a border); input focus ring `0 0 0 3px … 0.12`; primary-button hover `0 4px 16px … 0.20`; selected item (`.tipo-card.elegida`, `.dash-nav-item.activo`) accent border over `0.04`; and the per-event glow on the public grid's `.ticket[data-color-evento]` (`0 8px 40px … 0.18` at rest, `0.30` on hover with a `translateY(-4px)` lift, none when sold out or past) — that card has no surface/border/background of its own (see "FlashPass-style event grid" below), the glow *is* its edge. All of them are written `rgba(var(--acento-rgb), a)` — see "Per-event accent color" below — so they re-tint with the event. Secondary buttons deliberately cast no shadow: that reserve belongs to the primary.
